        ...
        <a href="{{ $banner->button_url ?: route('collections.index') }}"
            ...
            <div class="relative mx-auto flex min-h-[280px] max-w-7xl items-center px-4 py-10 sm:min-h-[360px] sm:px-6 lg:min-h-[430px] lg:px-8">
                <div class="max-w-xl text-white {{ ($banner->align ?? 'left') === 'right' ? 'ml-auto text-left lg:text-right' : '' }}">
                    <p class="inline-flex border border-white/50 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.24em] text-white/90">
                        {{ $banner->eyebrow }}
                    </p>
                    <h2 class="mt-5 text-4xl font-semibold leading-none tracking-normal sm:text-5xl lg:text-6xl">
                        {{ $banner->title }}
                    </h2>
                    <p class="mt-5 max-w-md text-sm leading-7 text-white/80 {{ ($banner->align ?? 'left') === 'right' ? 'lg:ml-auto' : '' }}">
                        {{ $banner->subtitle }}
                    </p>
                    <span class="mt-7 inline-flex items-center gap-3 bg-white px-6 py-3 text-xs font-bold uppercase tracking-[0.16em] text-brand-dark transition group-hover:bg-brand-secondary">
